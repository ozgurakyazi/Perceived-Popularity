...
if ( ! class_exists( 'TGM_Plugin_Activation' ) ) {
	...
	class TGM_Plugin_Activation {
		...
		public function init() {
			...
			// Load class strings.
			$this->strings = array(
				'page_title'                      => __( 'Install Required Plugins', 'savile-row' ),
				'menu_title'                      => __( 'Install Plugins', 'savile-row' ),
				'installing'                      => __( 'Installing Plugin: %s', 'savile-row' ),
				'oops'                            => __( 'Something went wrong.', 'savile-row' ),
				'notice_can_install_required'     => _n_noop( 'This theme requires the following plugin: %1$s.', 'This theme requires the following plugins: %1$s.', 'savile-row' ),
				'notice_can_install_recommended'  => _n_noop( 'This theme recommends the following plugin: %1$s.', 'This theme recommends the following plugins: %1$s.', 'savile-row' ),
				'notice_cannot_install'           => _n_noop( 'Sorry, but you do not have the correct permissions to install the %s plugin. Contact the administrator of this site for help on getting the plugin installed.', 'Sorry, but you do not have the correct permissions to install the %s plugins. Contact the administrator of this site for help on getting the plugins installed.', 'savile-row' ),
				'notice_can_activate_required'    => _n_noop( 'The following required plugin is currently inactive: %1$s.', 'The following required plugins are currently inactive: %1$s.', 'savile-row' ),
				'notice_can_activate_recommended' => _n_noop( 'The following recommended plugin is currently inactive: %1$s.', 'The following recommended plugins are currently inactive: %1$s.', 'savile-row' ),
				'notice_cannot_activate'          => _n_noop( 'Sorry, but you do not have the correct permissions to activate the %s plugin. Contact the administrator of this site for help on getting the plugin activated.', 'Sorry, but you do not have the correct permissions to activate the %s plugins. Contact the administrator of this site for help on getting the plugins activated.', 'savile-row' ),
				'notice_ask_to_update'            => _n_noop( 'The following plugin needs to be updated to its latest version to ensure maximum compatibility with this theme: %1$s.', 'The following plugins need to be updated to their latest version to ensure maximum compatibility with this theme: %1$s.', 'savile-row' ),
				'notice_cannot_update'            => _n_noop( 'Sorry, but you do not have the correct permissions to update the %s plugin. Contact the administrator of this site for help on getting the plugin updated.', 'Sorry, but you do not have the correct permissions to update the %s plugins. Contact the administrator of this site for help on getting the plugins updated.', 'savile-row' ),
				'install_link'                    => _n_noop( 'Begin installing plugin', 'Begin installing plugins', 'savile-row' ),
				'activate_link'                   => _n_noop( 'Begin activating plugin', 'Begin activating plugins', 'savile-row' ),
				'return'                          => __( 'Return to Required Plugins Installer', 'savile-row' ),
				'dashboard'                       => __( 'Return to the dashboard', 'savile-row' ),
				'plugin_activated'                => __( 'Plugin activated successfully.', 'savile-row' ),
				'activated_successfully'          => __( 'The following plugin was activated successfully:', 'savile-row' ),
				'complete'                        => __( 'All plugins installed and activated successfully. %1$s', 'savile-row' ),
				'dismiss'                         => __( 'Dismiss this notice', 'savile-row' ),
			);
		...
		protected function do_plugin_install() {
			...
			if ( isset( $_GET['plugin'] ) && ( isset( $_GET['tgmpa-install'] ) && 'install-plugin' === $_GET['tgmpa-install'] ) ) {
				...
				if ( empty( $complete ) ) {
					echo '<p>', sprintf( esc_html( $this->strings['complete'] ), '<a href="' . esc_url( self_admin_url() ) . '">' . esc_html__( 'Return to the Dashboard', 'savile-row' ) . '</a>' ), '</p>';
		...
		public function notices() {
			...
			if ( ! empty( $message ) ) {
				...
				foreach ( $message as $type => $plugin_groups ) {
					...
					$rendered .= '<p>' . sprintf( translate_nooped_plural( $this->strings[ $type ], $count, 'savile-row' ), $imploded, $count ) . '</p>';
				}

				// Setup variables to determine if action links are needed.
				$show_install_link  = $install_link ? '<a href="' . esc_url( add_query_arg( 'page', urlencode( $this->menu ), self_admin_url( $this->parent_slug ) ) ) . '">' . translate_nooped_plural( $this->strings['install_link'], $install_link_count, 'savile-row' ) . '</a>' : '';
				$show_activate_link = $activate_link ? '<a href="' . esc_url( add_query_arg( 'page', urlencode( $this->menu ), self_admin_url( $this->parent_slug ) ) ) . '">' . translate_nooped_plural( $this->strings['activate_link'], $activate_link_count, 'savile-row' ) . '</a>'  : '';
	...
	class TGMPA_List_Table extends WP_List_Table {
			...
			foreach ( $this->tgmpa->plugins as $plugin ) {
				...
				if ( ! empty( $plugin['source'] ) ) {
					if ( preg_match( '|^http(s)?://|', $plugin['source'] ) ) {
						...
						$table_data[ $i ]['source'] = __( 'Private Repository', 'savile-row' );
					}
					else {
						// The plugin is pre-packaged with the theme.
						$table_data[ $i ]['source'] = __( 'Pre-Packaged', 'savile-row' );
					}
				}
				// The plugin is from the WordPress repository.
				else {
					$table_data[ $i ]['source'] = __( 'WordPress Repository', 'savile-row' );
				}

				$table_data[ $i ]['type'] = isset( $plugin['required'] ) && $plugin['required'] ? __( 'Required', 'savile-row' ) : __( 'Recommended', 'savile-row' );

				if ( ! isset( $installed_plugins[ $plugin['file_path'] ] ) ) {
					$table_data[ $i ]['status'] = sprintf( '%1$s', __( 'Not Installed', 'savile-row' ) );
				}
				elseif ( is_plugin_inactive( $plugin['file_path'] ) ) {
					$table_data[ $i ]['status'] = sprintf( '%1$s', __( 'Installed But Not Activated', 'savile-row' ) );
				}
		...
		public function column_plugin( $item ) {
			...
			if ( ! isset( $installed_plugins[ $item['file_path'] ] ) ) {
				...
				$actions = array(
					'install' => sprintf(
						'<a href="%1$s">' . esc_attr_x( 'Install %2$s', '%2$s = plugin name in screen reader markup', 'savile-row' ) . '</a>',
			...
			elseif ( is_plugin_inactive( $item['file_path'] ) ) {
				...
				$actions = array(
					'activate' => sprintf(
						'<a href="%1$s">' . esc_attr_x( 'Activate %2$s', '%2$s = plugin name in screen reader markup', 'savile-row' ) . '</a>',
		...
		public function no_items() {
			...
			printf( wp_kses_post( __( 'No plugins to install or activate. <a href="%1$s">Return to the Dashboard</a>', 'savile-row' ) ), esc_url( self_admin_url() ) );
			echo '<style type="text/css">#adminmenu .wp-submenu li.current { display: none !important; }</style>';
		...
		public function get_columns() {
			...
			$columns = array(
				...
				'plugin' => __( 'Plugin', 'savile-row' ),
				'source' => __( 'Source', 'savile-row' ),
				'type'   => __( 'Type', 'savile-row' ),
				'status' => __( 'Status', 'savile-row' ),
			);
		...
		public function get_bulk_actions() {
			...
			$actions = array(
				'tgmpa-bulk-install'  => __( 'Install', 'savile-row' ),
				'tgmpa-bulk-activate' => __( 'Activate', 'savile-row' ),
			);
		...
		public function process_bulk_actions() {
			...
			if ( 'tgmpa-bulk-install' === $this->current_action() ) {
				...
				if ( empty( $plugin_installs ) ) {
					echo '<div id="message" class="error"><p>', esc_html__( 'No plugins are available to be installed at this time.', 'savile-row' ), '</p></div>';
					return false;
				}
			...
			if ( 'tgmpa-bulk-activate' === $this->current_action() ) {
				...
				if ( empty( $plugins_to_activate ) ) {
					echo '<div id="message" class="error"><p>', esc_html__( 'No plugins are available to be activated at this time.', 'savile-row' ), '</p></div>';
					return false;
				}
				...
				if ( is_wp_error( $activate ) ) {
					...
					printf( '<div id="message" class="updated"><p>%1$s %2$s.</p></div>', esc_html( _n( 'The following plugin was activated successfully:', 'The following plugins were activated successfully:', $count, 'savile-row' ) ), wp_kses_post( $imploded ) );
				}
			...
			if ( ! class_exists( 'TGM_Bulk_Installer' ) ) {
				...
				class TGM_Bulk_Installer extends WP_Upgrader {
					...
					public function install_strings() {
						...
						$this->strings['no_package']          = __( 'Install package not available.', 'savile-row' );
						$this->strings['downloading_package'] = __( 'Downloading install package from <span class="code">%s</span>&#8230;', 'savile-row' );
						$this->strings['unpack_package']      = __( 'Unpacking the package&#8230;', 'savile-row' );
						$this->strings['installing_package']  = __( 'Installing the plugin&#8230;', 'savile-row' );
						$this->strings['process_failed']      = __( 'Plugin install failed.', 'savile-row' );
						$this->strings['process_success']     = __( 'Plugin installed successfully.', 'savile-row' );
					...
					public function activate_strings() {
						...
						$this->strings['activation_failed']  = __( 'Plugin activation failed.', 'savile-row' );
						$this->strings['activation_success'] = __( 'Plugin activated successfully.', 'savile-row' );
			...
			if ( ! class_exists( 'TGM_Bulk_Installer_Skin' ) ) {
				...
				class TGM_Bulk_Installer_Skin extends Bulk_Upgrader_Skin {
					...
					public function add_strings() {
						...
						$this->upgrader->strings['skin_update_failed_error']  = __( 'An error occurred while installing %1$s: <strong>%2$s</strong>.', 'savile-row' );
						$this->upgrader->strings['skin_update_failed']        = __( 'The installation of %1$s failed.', 'savile-row' );

						// Automatic activation strings.
						if ( $this->tgmpa->is_automatic ) {
							$this->upgrader->strings['skin_upgrade_start']        = __( 'The installation and activation process is starting. This process may take a while on some hosts, so please be patient.', 'savile-row' );
							$this->upgrader->strings['skin_update_successful']    = __( '%1$s installed and activated successfully.', 'savile-row' ) . ' <a onclick="%2$s" href="#" class="hide-if-no-js"><span>' . __( 'Show Details', 'savile-row' ) . '</span><span class="hidden">' . __( 'Hide Details', 'savile-row' ) . '</span>.</a>';
							$this->upgrader->strings['skin_upgrade_end']          = __( 'All installations and activations have been completed.', 'savile-row' );
							$this->upgrader->strings['skin_before_update_header'] = __( 'Installing and Activating Plugin %1$s (%2$d/%3$d)', 'savile-row' );
						}
						// Default installation strings.
						else {
							$this->upgrader->strings['skin_upgrade_start']        = __( 'The installation process is starting. This process may take a while on some hosts, so please be patient.', 'savile-row' );
							$this->upgrader->strings['skin_update_successful']    = __( '%1$s installed successfully.', 'savile-row' ) . ' <a onclick="%2$s" href="#" class="hide-if-no-js"><span>' . __( 'Show Details', 'savile-row' ) . '</span><span class="hidden">' . __( 'Hide Details', 'savile-row' ) . '</span>.</a>';
							$this->upgrader->strings['skin_upgrade_end']          = __( 'All installations have been completed.', 'savile-row' );
							$this->upgrader->strings['skin_before_update_header'] = __( 'Installing Plugin %1$s (%2$d/%3$d)', 'savile-row' );
						}
					...
					public function bulk_footer() {
						...
						if ( empty( $complete ) ) {
							echo '<p>', sprintf( esc_html( $this->tgmpa->strings['complete'] ), '<a href="' . esc_url( self_admin_url() ) . '">' . esc_html__( 'Return to the Dashboard', 'savile-row' ) . '</a>' ), '</p>';
							echo '<style type="text/css">#adminmenu .wp-submenu li.current { display: none !important; }</style>';
						}
